...
export type FocusArea = 'list' | 'editor' | 'file-tabs' | 'settings' | 'terminal' | 'terminal-tabs' | 'file-search' | 'note-search' | 'file-tree';

export const focusArea = writable<FocusArea>('list');

// Order: search → list → editor → terminal → terminal-tabs → file-tabs (cycle)
...
export function nextFocusArea(current: FocusArea): FocusArea {
    ...
    const areas = ['search', 'list', 'editor', 'terminal', 'terminal-tabs', 'file-tabs'];
